...
from recipedb import Base, Recipe, User
...
@app.route('/recipes/')
def showRecipes():
	recipes = session.query(Recipe).order_by(asc(name))
	return render_template('recipes.html', recipes=recipes, login_session=login_session)


# Show single recipe
#@app.route('/recipes/<string:recipe.name>')
...
def oneUserRecipe(creator_id):
	...
	if 'username' not in login_session:
		return render_template('publiconeuser.html', creator=creator, recipes=recipes,
			login_session=login_session)
	else:
		user_id = getUserID(login_session['email'])
		if creator_id == user_id:
			return render_template('oneuser.html', creator=creator, recipes=recipes,
				login_session=login_session)
		else:
			return render_template('publiconeuser.html', creator=creator, recipes=recipes,
				login_session=login_session)
...
def newRecipe():
	...
	if request.method == 'POST':
		newItem = Recipe(name=request.form['name'], instructions=request.form[
			'instructions'], type=request.form['type'], picture=request.form['picture'],
				user_id=currentUser)
		session.add(newItem)
		session.commit()
	...
	if 'username' not in login_session:
		...
		if user_id == editedItem.user_id:
			if request.method == 'POST':
				...
				return render_template('editrecipe.html', login_session=login_session,
	...
	if 'username' not in login_session:
		...
		if user_id == itemToDelete.user_id:
			if request.method == 'POST':
				...
				return render_template('deleterecipe.html', login_session=login_session, item=itemToDelete)
		else:
